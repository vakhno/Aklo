...
import * as roomService from "@shared/mongo/services/room.service";
import * as roomCacheService from "@shared/redis/services/room-cache.service";
import { createRoomCache } from "@shared/redis/services/room-cache.service";
import { RoomInputSchema } from "@shared/schemas/room";
import { getCookies } from "@shared/sockets/utils/get-cookies";
...
export async function updateRoom(req: Request, res: Response) {
	try {
		...
		const updated = await roomService.updateRoom(String(id), req.body);
...
export async function getRoomUsers(req: Request, res: Response) {
	try {
		...
		const cache = await roomCacheService.getRoomCache(String(id));
...
export async function removeRoomUser(req: Request, res: Response) {
	try {
		...
		await roomCacheService.leftRoomCache(String(id), String(userId), undefined);
		await roomService.leftRoom(String(id), undefined);
...
export async function createRoom(req: Request, res: Response) {
	try {
		...
		const userId = (req as { session?: { user: { id: string } } }).session?.user?.id;
		...
		const createdRoom = await roomService.createRoom(parsedData, userId);
...
export async function getRoom(req: Request, res: Response) {
	try {
		...
		const room = await roomService.getRoom(String(id));
...
export async function resetRoom(req: Request, res: Response) {
	try {
		...
		await roomService.resetRoom(String(id));
		await roomCacheService.resetRoomCache(String(id));
...
export async function deleteRoom(req: Request, res: Response) {
	try {
		...
		await roomCacheService.deleteRoomCache(String(id));
		await roomService.deleteRoom(String(id));

		req.app.get("io").to(String(id)).emit("room-deleted", { roomId: String(id) });
...
export async function joinRoom(req: Request, res: Response) {
	try {
		...
		const creator = cookies?.[String(id)];

		await roomService.joinRoom(String(id), creator);
...
export async function leftRoom(req: Request, res: Response) {
	try {
		...
		await roomService.leftRoom(String(id), creator);
...
export async function checkIsCreator(req: Request, res: Response) {
	try {
		...
		const cookies = getCookies({ req });
		const isCreator = !!cookies[String(id)] && cookies[String(id)] === creatorId;
