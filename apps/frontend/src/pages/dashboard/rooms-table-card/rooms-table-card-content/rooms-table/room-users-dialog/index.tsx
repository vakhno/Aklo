...
import { useAdminGetRoomUsers, useAdminRemoveRoomUser } from "@shared/queries";
...
export default function RoomUsersDialog({ roomId, open, onOpenChange }: RoomUsersDialogProps) {
	...
	const { data, isLoading } = useAdminGetRoomUsers({ id: roomId, apiBaseUrl, options: { enabled: open } });
	const removeMutation = useAdminRemoveRoomUser();

	const handleRemove = (userId: string) => {
		removeMutation.mutate(
			{ apiBaseUrl, roomId, userId },
			{
