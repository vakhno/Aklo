...
interface RoomTableColumnsOptions {
	onEdit: (room: RoomDocLeanType) => void;
	onDelete: (room: RoomDocLeanType) => void;
	onUsers: (roomId: string) => void;
}

export const getColumns = ({ onEdit, onDelete, onUsers }: RoomTableColumnsOptions): ColumnDef<RoomDocLeanType>[] => [
	{
		accessorKey: "_id",
		header: "Id",
		size: 260
	...
	{
		id: "language",
		accessorFn: row => typeof row.language === "object" ? (row.language as unknown as LanguageDocLeanType).name : "",
		header: "Language",
		size: 120
	},
	{
	...
	},
	{
		...
		cell: ({ row }) => (
			<div className="flex gap-1">
				<Button variant="ghost" size="icon" onClick={() => onUsers(String(row.original._id))} title="View users">
