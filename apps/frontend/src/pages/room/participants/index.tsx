import { useCheckIsCreator, useGetRoom, useIsAvailableToVisit } from "@shared/queries";
import { useNavigate } from "@tanstack/react-router";
import { useEffect, useState } from "react";

import LogoSpinner from "@/assets/icon/logo-spinner.svg?react";
import Streams from "@/pages/room/participants/streams";
...
function Participants({ roomId }: ParticipantsProps) {
	...
	const { data: room, isPending: isPendingGetRoom, isError: isErrorGetRoom, isRefetching: isRefetchingGetRoom, isRefetchError: isRefetchErrorGetRoom, refetch: refetchGetRoom } = useGetRoom({ apiBaseUrl, id: roomId, options: roomQueryOptions });
	const { data: isAvailableToVisit, isPending: isPendingIsAvailableToVisit, isError: isErrorIsAvailableToVisit } = useIsAvailableToVisit({ apiBaseUrl, id: roomId, options: { ...roomQueryOptions, enabled: !!room } });
	const { data: isCreator, isPending: isPendingCheckIsCreator, isError: isErrorCheckIsCreator } = useCheckIsCreator({ apiBaseUrl, id: roomId, options: { ...roomQueryOptions, enabled: !!room } });
