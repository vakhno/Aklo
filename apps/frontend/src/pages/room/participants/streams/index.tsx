...
import { useDeleteRoom } from "@shared/queries";
...
const Streams = ({ room, isCreator, isAvailableToVisit, isErrorCheckIsCreator, isErrorIsAvailableToVisit }: StreamsProps) => {
	...
	const { remotePeers, isKicked, isFailed, isDeleted, handleKick, initSocket } = useWebRTC({ roomId: String(room._id), localStream: combinedStream });
	...
	const onHandleDeleteClick = async () => {
		await deleteRoom({ apiBaseUrl: import.meta.env.VITE_API_URL, id: String(room._id) });
