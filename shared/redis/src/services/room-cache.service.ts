import type { RoomCacheType, RoomDocLeanType } from "@shared/mongo/types/room";

import { RoomModel } from "@shared/mongo/room";

import { ROOM_PREFIX } from "../constants/redis";
...
export const createRoomCache = async (roomId: string): Promise<void> => {
	try {
		...
		await getRedisClient().hSet(key, convertObjToRedisHash(roomCache));
	}
	catch (error) {
